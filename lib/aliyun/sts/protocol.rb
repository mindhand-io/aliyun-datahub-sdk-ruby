  ...
  module STS
    ...
    class Protocol
      ...
      def do_request(params)
        ...
        r = Typhoeus::Request.new((@config.endpoint || ENDPOINT), method: :post, body: query)
        r.on_complete do |resp|
          if resp.code >= 300
            ...
            logger.debug("Received HTTP response, code: #{resp.code}, headers: "\
                   "#{resp.headers}, body: #{resp.body}")
            return resp.body
          end
        end
        r.run
